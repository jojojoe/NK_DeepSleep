...
open class CircularSlider: UIControl {
    ...
    */
    ...
    //New Add
    open var backTrackDashLengths: [CGFloat] = [0,0]
    open var frontTrackDashLengths: [CGFloat] = [0,0]
    open var singleClickThumbBlock: (()->Void)?
    open var doubleClickThumbBlock: (()->Void)?
    
    var isBeginClickThumb: Bool = false
    var isContinueTracking: Bool = false
    var beginTouchPointValue: CGFloat = 0
    var currentThumbOriginPoint: CGPoint = CGPoint.zero
//    var doubleTapCheckingCount: Int = 0
...
//        isBeginClickThumb
        ...
        debugPrint("beginTracking touchPosition = \(touchPosition)")
        
        if (abs(currentThumbOriginPoint.x - touchPosition.x) < 10) && (abs(currentThumbOriginPoint.y - touchPosition.y) < 10) {
            isBeginClickThumb = true
        } else {
            isBeginClickThumb = false
        }
        
        
        let startPoint = CGPoint(x: bounds.center.x, y: 0)
        let value = newValue(from: endPointValue, touch: touchPosition, start: startPoint)
        beginTouchPointValue = value
        
        debugPrint("circular thumb: isBeginClickThumb = \(isBeginClickThumb)")
    ...
    override open func continueTracking(_ touch: UITouch, with event: UIEvent?) -> Bool {
        ...
        if endPointValue - beginTouchPointValue > 5 {
            isContinueTracking = true
        }
        
        sendActions(for: .valueChanged)
        debugPrint("circular thumb: continueTracking isContinueTracking = \(isContinueTracking)")
        return true
    }
    ...
    open override func endTracking(_ touch: UITouch?, with event: UIEvent?) {
        ...
        if let touchPosition = touch?.location(in: self) {
            debugPrint("circular thumb:endTracking touchPosition = \(touchPosition)")
            debugPrint("circular thumb:isContinueTracking = \(isContinueTracking)")
            debugPrint("circular thumb:isBeginClickThumb = \(isBeginClickThumb)")
            if isContinueTracking == false && isBeginClickThumb {
                self.singleClickThumbBlock?()

            } else {

            }
            
            isContinueTracking = false
