...
extension CircularSlider {
     ...
     */
    internal static func drawArc(withArc arc: Arc, dashLengths:[CGFloat] = [0,0], lineWidth: CGFloat = 2, mode: CGPathDrawingMode = .fillStroke, inContext context: CGContext) {
    ...
    internal func drawCircularSlider(inContext context: CGContext) {
        ...
        CircularSlider.drawArc(withArc: sliderArc, dashLengths: backTrackDashLengths, lineWidth: lineWidth, inContext: context)
//        CircularSlider.drawArc(withArc: sliderArc, lineWidth: backtrackLineWidth, inContext: context)
    ...
    internal func drawFilledArc(fromAngle startAngle: CGFloat, toAngle endAngle: CGFloat, inContext context: CGContext) {
        ...
        CircularSlider.drawArc(withArc: arc, dashLengths: frontTrackDashLengths, lineWidth: lineWidth, mode: .stroke, inContext: context)
//        CircularSlider.drawArc(withArc: arc, lineWidth: lineWidth, mode: .stroke, inContext: context)
    ...
    internal func drawThumbAt(_ angle: CGFloat, with image: UIImage? = nil, inContext context: CGContext) -> CGPoint {
        ...
        currentThumbOriginPoint = thumbOrigin
//        debugPrint("drawThumbAt thumbOrigin = \(thumbOrigin)")
        if let image = image {
...
//        CircularSlider.drawArc(withArc: thumbArc, lineWidth: thumbLineWidth, inContext: context)
        CircularSlider.drawArc(withArc: thumbArc, dashLengths: frontTrackDashLengths, lineWidth: lineWidth, inContext: context)
        return thumbOrigin
    }
