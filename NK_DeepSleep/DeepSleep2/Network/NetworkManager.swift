...
//                let model = response.data.mapModel(DeepSleepResource.self)
...
//                if jsonData[RESULT_CODE].stringValue == "success" {
//                    completion(String(data: response.data, encoding: String.Encoding.utf8)!)
//                } else {
//                    print("status不为success HUD显示后台返回message ："+"\(jsonData[RESULT_MESSAGE].stringValue)")
//                    if let faillHandler = failed {
//                        faillHandler(String(data: response.data, encoding: String.Encoding.utf8)!)
//                    }
//                }
        ...
        case let .failure(error):
            guard let _ = error as? CustomStringConvertible else {
                //网络连接失败，提示用户
                print("网络连接失败")
                break
            }
...
extension Data {
    func mapModel<T: Codable>(_ type: T.Type) -> T? {
